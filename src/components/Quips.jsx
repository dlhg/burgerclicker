...
const Quips = (props) => {
  const determineQuipLevel = () => {
    const { totalBurgersProduced } = props;

    for (const { threshold, quips } of quipLevelThresholds.reverse()) {
      if (totalBurgersProduced >= threshold) {
        return {
          level: quipLevelThresholds.indexOf({ threshold, quips }),
          quips,
        };
      }
    }
    ...
    return { level: 0, quips: level1quips };
  };
  const [quipLevel, setQuipLevel] = useState(determineQuipLevel());

  useEffect(() => {
    const quipInterval = setInterval(() => {
      setQuipLevel(determineQuipLevel());
    }, 7000);
